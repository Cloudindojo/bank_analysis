...
#plt.savefig('static/Ages_by_Occupation.png')
#------------------
# In[11]:
...
#iplot(fig21, filename='polar-area-chart')
graphJSON2 = json.dumps(fig21, cls=plotly.utils.PlotlyJSONEncoder)
#plt.savefig('static/balance_status.png')
#------------------------
# In[12]:
...
fig4.append_trace(married_dist, 2, 1)
fig4.append_trace(divorced_dist, 3, 1)


fig4['layout'].update(showlegend=True, title="Price Distributions by Marital Status",
                    height=1000, width=800)

#iplot(fig4, filename='custom-sized-subplot-with-subplot-titles')
graphJSON4 = json.dumps(fig4, cls=plotly.utils.PlotlyJSONEncoder)
#plt.savefig('static/balance_by_marital.png')
#-------------------------------
# In[17]:


# Notice how divorced have a considerably low amount of balance.
fig3 = ff.create_facet_grid(
    df,
    x='duration',
    y='balance',
    color_name='marital',
...
#iplot(fig3, filename='facet - custom colormap')
graphJSON5 = json.dumps(fig3, cls=plotly.utils.PlotlyJSONEncoder)
#----------------------
...
graphJSON6 = json.dumps(fig2, cls=plotly.utils.PlotlyJSONEncoder)
#--------------------------------
# In[21]:
...
#iplot(fig1, filename='lowest-oecd-votes-cast')
graphJSON7 = json.dumps(fig1, cls=plotly.utils.PlotlyJSONEncoder)
#plt.savefig('static/Loan_to_married.png')
#-----------------------------
# In[26]:
...
y_clf_predict = grad_clf.predict([some_instance])
y_clf_predict

# In[70]:


y_log = log_reg.fit(X_train, y_train)


# In[71]:


y_logistic = log_reg.decision_function([some_instance])
y_logistic
...
y_log_predict = y_log.predict([some_instance])
y_log_predict
...
for f in range(X_train.shape[1]):
    ll.append("%d. feature %d (%f)" % (f + 1, indices[f], importances[indices[f]]))
...
@app.route('/home')
def home():
    return render_template('home.html', s9=df.describe().to_html())

@app.route('/train')
def train():
    return render_template('train.html', s=s.to_html())

@app.route('/test')
def test():
    return render_template('test.html', s1=df_results.to_html())
...
def deepanalysis():
    return render_template('deepanalysis.html', j1=graphJSON, j2=graphJSON1, j3=graphJSON2, j4=graphJSON3, j5=graphJSON4, j6=graphJSON5, j7=graphJSON6)

@app.route('/featureanalysis')
def featureanalysis():
    return render_template('featureanalysis.html',ll=ll)
